...
export default function Profile() {
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ paddingBottom:40 }}>
      ...
      <View style={s.card}>
        <View style={s.avatar}><Text style={s.avatarText}>{twinName?.charAt(0)?.toUpperCase()||'?'}</Text></View>
      ...
      <View style={s.section}>
        ...
        {editing ? (
          <>
            <View style={s.row}><User size={16} stroke="#6B21A8" /><TextInput style={s.input} placeholder={t.name} value={name} onChangeText={setName} /></View>
            <View style={s.row}><Phone size={16} stroke="#6B21A8" /><TextInput style={s.input} placeholder={t.phone} value={phone} onChangeText={setPhone} keyboardType="phone-pad" /></View>
            <View style={s.btnRow}>
          ...
          <>
            <View style={s.row}><User size={16} stroke="#6B21A8" /><Text style={s.value}>{profile.full_name||'—'}</Text></View>
            <View style={s.row}><Mail size={16} stroke="#6B21A8" /><Text style={s.value}>{profile.email||'—'}</Text></View>
            <View style={s.row}><Phone size={16} stroke="#6B21A8" /><Text style={s.value}>{profile.phone||'—'}</Text></View>
      ...
      <View style={s.section}>
        ...
        <View style={s.row}><Crown size={16} stroke="#6B21A8" /><Text style={s.label}>{t.tier}</Text><Text style={s.value}>{tier}</Text></View>
        <View style={s.row}><Zap size={16} stroke="#6B21A8" strokeWidth={2} /><Text style={s.label}>{t.messagesLeft}</Text><Text style={s.value}>{usage.messages||0}</Text></View>
        <View style={s.row}><MessageSquare size={16} stroke="#6B21A8" /><Text style={s.label}>{t.totalMessages}</Text><Text style={s.value}>{profile.total_messages||0}</Text></View>
      </View>
      ...
      <TouchableOpacity style={[s.btn, s.outlineBtn]} onPress={handleLogout}><LogOut size={16} stroke="#6B21A8" /><Text style={[s.btnText,{color:'#6B21A8'}]}>{t.logout}</Text></TouchableOpacity>
