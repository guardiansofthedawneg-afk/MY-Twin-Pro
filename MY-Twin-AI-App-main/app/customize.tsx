import { SafeAreaView, View, Text, StyleSheet, ScrollView, TextInput, TouchableOpacity, Alert } from 'react-native';
import { useState } from 'react';
import { useTwinStore, TwinStyle, TwinGender } from '../store/useTwinStore';
...
export default function Customize() {
  const { twinName, twinGender, twinStyle, setTwinName, setTwinGender, setTwinStyle, lang, theme } = useTwinStore();
  const isAr = lang === 'ar';
  ...
  const styleLabels = STYLES[lang] || STYLES['ar'];
  const styleKeys = Object.keys(styleLabels) as TwinStyle[];
  ...
  return (
    <SafeAreaView style={[s.safe, isDark && { backgroundColor: '#1A1A1A' }]}>
      <ScrollView style={s.container} contentContainerStyle={{ padding: 20 }}>
        ...
        <Palette size={40} stroke={isDark ? '#D8B4FE' : '#6B21A8'} style={{ alignSelf: 'center', marginBottom: 24 }} />
        ...
        <View style={[s.inputRow, isDark && { backgroundColor: '#2A2A2A', borderColor: '#444' }]}>
          ...
          <TextInput style={[s.input, isDark && { color: '#FFF' }]} value={name} onChangeText={setName} placeholder={t('أدخل الاسم','Enter name')} placeholderTextColor="#999" maxLength={30} />
        </View>

        <Text style={[s.label, isDark && { color: '#CCC' }]}>{t('النوع','Gender')}</Text>
        <View style={s.optionRow}>
          {(['female', 'male'] as TwinGender[]).map(g => (
            <TouchableOpacity key={g} style={[s.optionBtn, gender === g && s.activeOption, isDark && { borderColor: gender === g ? '#D8B4FE' : '#444' }]} onPress={() => setGender(g)}>
              <Text style={[s.optionText, gender === g && s.activeOptionText, isDark && { color: gender === g ? '#FFF' : '#CCC' }]}>{g === 'female' ? t('🌸 أنثى','🌸 Female') : t('⚡ ذكر','⚡ Male')}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={s.optionRow}>
          {styleKeys.map(sk => (
            <TouchableOpacity key={sk} style={[s.optionBtn, style === sk && s.activeOption, isDark && { borderColor: style === sk ? '#D8B4FE' : '#444' }]} onPress={() => setStyle(sk)}>
              <Text style={[s.optionText, style === sk && s.activeOptionText, isDark && { color: style === sk ? '#FFF' : '#CCC' }]}>{styleLabels[sk]}</Text>
            </TouchableOpacity>
          ))}
        </View>
...
const s = StyleSheet.create({
  ...
  optionBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, borderWidth: 1.5, borderColor: '#E0D9F5', backgroundColor: '#FFF' },
  activeOption: { borderColor: '#6B21A8', backgroundColor: '#F3F0FF' },
  optionText: { fontSize: 13, color: '#666', fontWeight: '500' },
  activeOptionText: { color: '#6B21A8', fontWeight: '700' },
