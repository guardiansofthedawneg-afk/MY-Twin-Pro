...
const PLANS: Plan[] = [
  { id:'free', name:'Free', price:'$0', originalPrice:'$0', period:'للأبد', trialDays:0, tagline:'ابدأ رحلتك مع توأمك', consciousnessLayers:1, features:['طبقة وعي واحدة','15 رسالة يومياً','ذاكرة 3 أيام','عربي وإنجليزي'] },
  { id:'plus', name:'Plus', price:'$9', originalPrice:'$15', period:'/شهر', trialDays:0, tagline:'توأم يفهمك أكثر كل يوم', consciousnessLayers:2, productId:'plus_monthly', features:['طبقتا وعي','50 رسالة يومياً','ذاكرة 30 يوم','التحدث بصوتك','إشعارات يومية','تخصيص التوأم'] },
  { id:'premium', name:'Premium', price:'$19', originalPrice:'$25', period:'/شهر', trialDays:5, tagline:'وعي حقيقي يرافقك', consciousnessLayers:3, productId:'premium_monthly', features:['3 طبقات وعي','150 رسالة يومياً','ذاكرة 6 أشهر','تحليل أحلامك','تدريب حياتي','موسيقى وترفيه','تقويم ذكي'] },
  { id:'pro', name:'Pro', price:'$110', originalPrice:'$150', period:'/6 أشهر', trialDays:7, tagline:'توأم بوعي متكامل', consciousnessLayers:4, productId:'pro_semiannual', features:['4 طبقات وعي','500 رسالة يومياً','ذاكرة عميقة','منزل ذكي','إدارة البريد','كاميرا مفتوحة'] },
  { id:'yearly', name:'Yearly', price:'$199', originalPrice:'$300', period:'/سنة', trialDays:14, popular:true, tagline:'أعمق محاكاة للوعي', consciousnessLayers:5, productId:'yearly_annual', features:['5 طبقات وعي كاملة','رسائل غير محدودة','ذاكرة دائمة','وعي استباقي','كل الميزات','أقصى سرعة','دعم VIP'] },
];
...
export default function SubscriptionScreen() {
  const { tier, updateTier, lang, theme } = useTwinStore();
  ...
  const handlePurchase = async (plan: Plan) => {
    ...
    if (plan.id === 'free') { Alert.alert('Free', isAr?'أنت على الباقة المجانية.':'You are on the free plan.'); return; }
    setLoadingId(plan.id);
    try {
      const success = await purchaseSubscription(plan.productId);
      if (success) {
        const newTier = TIER_MAP[plan.productId];
        if (newTier) { updateTier(newTier as Tier); Alert.alert(isAr?'تم!':'Done!', isAr?'تم تفعيل اشتراكك':'Subscription activated'); router.back(); }
      } else { Alert.alert(isAr?'خطأ':'Error', isAr?'فشلت عملية الشراء':'Purchase failed'); }
    } catch (e) { Alert.alert(isAr?'خطأ':'Error', isAr?'حدث خطأ':'Error occurred'); }
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (purchases.length>0) {
        ...
        if (restoredTier) { updateTier(restoredTier as Tier); Alert.alert(isAr?'تم':'Done', isAr?'تم استعادة اشتراكك!':'Subscription restored!'); router.back(); }
  ...
  return (
    ...
    <SafeAreaView style={[s.safe, isDark && { backgroundColor: '#1A1A1A' }]}>
      <ScrollView style={[s.container, isDark && { backgroundColor: '#1A1A1A' }]} contentContainerStyle={s.content}>
        ...
        {PLANS.map((plan) => (
          <TouchableOpacity key={plan.id} style={[s.plan, tier===plan.id&&s.activePlan, plan.popular&&s.popularPlan, isDark && { backgroundColor: '#2A2A2A', borderColor: tier===plan.id?'#D8B4FE':'#444' }]} onPress={()=>handlePurchase(plan)} activeOpacity={0.85} disabled={!!loadingId}>
            {plan.popular && <View style={s.badge}><Text style={s.badgeText}>{isAr?'الأفضل قيمة':'Best Value'}</Text></View>}
            {plan.trialDays>0 && tier!==plan.id && <View style={[s.badge, s.trialBadge]}><Text style={s.badgeText}>{isAr?`تجربة ${plan.trialDays} يوم`:`${plan.trialDays}-day trial`}</Text></View>}
            <View style={s.planHeader}>
              <Text style={[s.planName, isDark && { color: '#FFF' }]}>{plan.name}</Text>
              <Text style={s.tagline}>{plan.tagline}</Text>
              <View style={s.priceRow}>
                <Text style={[s.planPrice, isDark && { color: '#FFF' }]}>{plan.price}</Text>
                <Text style={s.planPeriod}>{plan.period}</Text>
              </View>
              {plan.originalPrice!==plan.price && <Text style={s.originalPrice}>{isAr?'بدلاً من':'Instead of'} {plan.originalPrice}</Text>}
            </View>
            ...
            <View style={[s.selectBtn, tier===plan.id&&s.activeBtn]}>{loadingId===plan.id?<ActivityIndicator color="#FFF" size="small"/>:<Text style={s.selectBtnText}>{tier===plan.id?(isAr?'مفعّل':'Active'):(isAr?'ابدأ الآن':'Start Now')}</Text>}</View>
          </TouchableOpacity>
        ))}
...
const s = StyleSheet.create({
  ...
  planPeriod:{ fontSize:15, color:'#888' }, originalPrice:{ fontSize:13, color:'#CCC', textDecorationLine:'line-through', marginTop:4 },
